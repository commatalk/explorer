...
AddressController.prototype.getAddressSummary = function(address, options, callback) {
  // change 1: get address balance
  request("http://localhost:5000/api/v1/address/" + address, { json: true }, (err, res2, body) => {
    if (err) { return callback(err); }

console.log(body);
    var balance = body.body[0]["balance"];
    var transformed = {
      addrStr: address,
      balance: balance,
      balanceSat: balance * 1e8,
      totalReceived: body.body[0]["receivedBalance"],
      totalReceivedSat: body.body[0]["receivedBalance"] * 1e8,
      totalSent: body.body[0]["sentBalance"],
      totalSentSat: body.body[0]["sentBalance"] * 1e8,
      unconfirmedBalance: body.body[0]["unconfirmBalance"],
      unconfirmedBalanceSat: body.body[0]["unconfirmBalance"] * 1e8,
      unconfirmedTxApperances: body.body[0]["txCount"],  // will be deprecated in a future update
      unconfirmedAppearances: 0,
      txApperances: body.body[0]["txCount"], // will be deprecated in a future update
      txAppearances: body.body[0]["txCount"],
      transactions: body.body[0]["txCount"]
    };
