...
BlockController.prototype.list = function(req, res) {
  ...
  // change 1: get latest block from other service
  request("http://localhost:5000/api/v1/getblockbytime/" + gte + "/" + lte, { json: true }, (err, res2, body) => {
...
BlockController.prototype.getBlockReward = function(prevHash, cb) {
  ...
  self.getPreviousBlock(prevHash, function(err, info) {
    ...
    if(info===null){
        //Genesis reward is 250.
        cb(null, parseFloat("250".toString(10)).toFixed(8));
    }
    else{
	...
	if (nPrevHeight < 1) {
		...
		reward = 1;
	} else if (nPrevHeight < 151200) {
		reward = 2000;
	} else if (nPrevHeight < 410400) {
		reward = 1000;
	} else if (nPrevHeight < 1058400) {
		reward = 500;
	} else if(nPrevHeight < 3132000) {
		reward = 300;
	} else if(nPrevHeight < 7279200) {
		reward = 200;
	} else if(nPrevHeight < 20575668) {
		reward = 66;
	} else {
		reward = 0;
	}
