    ...
    self.node.getBlock(hash, function(err, block) {
      ...
      self.node.services.dashd.getBlockHeader(hash, function(err, info) {
        ...
        self.getBlockReward(blockResult.previousblockhash, function(err,reward) {
          ...
          request("http://localhost:3001/insight-api/block-index/" + blockResult.height, { json: true }, (err, res2, body) => {
            if (err) { next(); }

            blockResult["hash"] = body.blockHash;

            request("http://localhost:3001/insight-api/txs?block=" + hash, { json: true }, (err, res2, body) => {
              if (err) { next(); }

              blockResult["txs"] = body.txs;
              next();
            });
          });
...
BlockController.prototype.list = function(req, res) {
  ...
  if (req.query.limit == '5') {
    request("http://localhost:5000/api/v1/getblocknew/10", { json: true }, (err, res2, body) => {
...
BlockController.prototype.getBlockReward = function(prevHash, cb) {
  ...
  self.getPreviousBlock(prevHash, function(err, info) {
    ...
    if(info===null){
        ...
        if(nPrevHeight == 0) {
          reward = 4742057;
        } else if(nPrevHeight >= 0 && nPrevHeight < 1440) {
          reward = 4742009;
        } else if(nPrevHeight >= 1440 && nPrevHeight < 21600) {
          reward = 1;
        } else if(nPrevHeight >= 21600 && nPrevHeight < 151200) {
          reward = 2000;
        } else if(nPrevHeight >= 151200 && nPrevHeight < 410400) {
          reward = 1000;
        } else if(nPrevHeight >= 410400 && nPrevHeight < 1058400) {
          reward = 500;
        } else if(nPrevHeight >= 1058400 && nPrevHeight < 3132000) {
          reward = 300;
        } else if(nPrevHeight >= 3132000 && nPrevHeight < 7279200) {
          reward = 200;
        } else if(nPrevHeight >= 7279200 && nPrevHeight < 20575668) {
          reward = 66;
        } else {
          reward = 0;
        }
